...
public class ServiceBusProcessorIntegrationList2Impl implements ServiceBusProcessorIntegration {

    private static final Logger log = LoggerFactory.getLogger(ServiceBusProcessorIntegrationList2Impl.class);

    @Override
    public boolean supports(String queueName) {
        return queueName.equals("list2-queue-sample");
    }

    @Override
    public void processMessage(ServiceBusMessageContextIntegration mensagem) {
        log.info("Processing list2 message {}", mensagem);
