...
public class ServiceBusProcessorIntegrationList3Impl implements ServiceBusProcessorIntegration {

    private static final Logger log = LoggerFactory.getLogger(ServiceBusProcessorIntegrationList3Impl.class);

    @Override
    public boolean supports(String queueName) {
        return queueName.equals("list3-queue-sample");
    }

    @Override
    public void processMessage(ServiceBusMessageContextIntegration mensagem) {
        log.info("Processing list3 message {}", mensagem);
